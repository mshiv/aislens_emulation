...
for i in iceShelfRegions:
    ...
    mlt_rgrs.to_netcdf(main_dir / DIR_basalMeltObs_Interim / '{}_rgrs_vals.nc'.format(icems.name.values[i]))
    mlt_prd = mlt_rgrs.slope*h_mean + mlt_rgrs.intercept
    # flx_ddrft = flx - flx_prd
    mlt_prd.to_netcdf(main_dir / DIR_basalMeltObs_Interim / '{}_rgrs.nc'.format(icems.name.values[i]))
